...
class RealTimeRecorded extends Simulation {
	...
	val scn = scenario("RealTimeRecorded")
		.exec(http("request_0")
			.get("/login")
		...
		.pause(3)
		.exec(http("request_6")
			.get("/incidencias/kafka-messages")
	...
	setUp(scn.inject(rampUsers(10) over(50 seconds))).protocols(httpProtocol)
}